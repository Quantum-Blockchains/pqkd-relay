...
impl EtsiServer {
    pub async fn build(state: AppStateEtsi, pqkd: &Pqkd) -> EtsiServer {
        let app = Router::new()
            ...
            .with_state(state);
        let listener = tokio::net::TcpListener::bind(format!("0.0.0.0:{}", pqkd.port()))
            .await
            .unwrap();
...
async fn send_keys(
    ...
    let pqkd = state
        .pqkds()
        .iter()
        .find(|p| p.sae_id() == state.sae_id())
        .unwrap();

    let position = path.iter().position(|i| i == pqkd.sae_id()).unwrap();
...
async fn h(state: AppStateEtsi, mut req: Request) -> Result<Response, StatusCode> {
